...
public class ClientRMIGUI extends JFrame implements ActionListener{
	...
	public JPanel getTextPanel(){
		String welcome = "Bienvenue entrez votre nom et appuyez sur Demarrer pour commencer\n";
	...
	public JPanel getUsersPanel(){
		...
		String  userStr = " Utilisateurs Courants      ";
		
		JLabel userLabel = new JLabel(userStr, JLabel.CENTER);
		userPanel.add(userLabel, BorderLayout.NORTH);	
		userLabel.setFont(new Font("Meiryo", Font.PLAIN, 16));

		String[] noClientsYet = {"Pas d'autres utilisateurs"};
	...
	public JPanel makeButtonPanel() {		
        ...
        privateMsgButton = new JButton("Envoyer PV");
		...
		try {
			...
			if(e.getSource() == startButton){
				...
				if(name.length() != 0){
					...
					textField.setText("");
					textArea.append("l'utilisateur : " + name + " rejoint la discution...\n");							
					getConnected(name);
					...
					JOptionPane.showMessageDialog(frame, "Entrer votre nom pour commencer");
				}
			}
			...
			if(e.getSource() == privateMsgButton){
				...
				for(int i=0; i<privateList.length; i++){
					System.out.println("index sélectionné :" + privateList[i]);
				}
				message = textField.getText();
				textField.setText("");
	...
	private void sendPrivate(int[] privateList) throws RemoteException {
		String privateMessage = "[PV dpuis " + name + "] :" + message + "\n";
